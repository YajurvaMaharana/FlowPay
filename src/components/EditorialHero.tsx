      ...
      >
        ...
        <header className="relative z-20 w-full px-6 sm:px-10 py-5 sm:py-7 flex items-center justify-between border-b border-white/10 backdrop-blur-md bg-neutral-950/20">
          ...
          <div className="flex items-center gap-3 sm:gap-4">
            ...
            {/* Unified Authentication Trigger or Avatar */}
            {!user.isAuthenticated ? (
              <button
                id="hero-signin-btn"
                onClick={() => onOpenAuthModal('login')}
                className="px-3.5 py-1.5 rounded-xl border border-white/20 bg-black/60 hover:bg-black/80 text-white text-xs font-semibold backdrop-blur-md transition-all shadow-md active:scale-95 flex items-center gap-1.5 group"
              >
                <User className="w-3.5 h-3.5 text-zinc-300 group-hover:text-white transition-colors" />
                <span>Sign In</span>
              </button>
            ) : (
                ...
                {isProfileOpen && (
                  ...
                  >
                    ...
                    <div className="space-y-0.5 text-xs">
                      ...
                      <button
                        id="hero-profile-menu-edit-profile"
                        onClick={() => {
                          setIsProfileOpen(false);

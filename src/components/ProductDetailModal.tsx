...
export const ProductDetailModal: React.FC<ProductDetailModalProps> = ({
  ...
  return (
    <div id="product-modal-overlay" className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-fadeIn">
      <div 
      ...
      >
        ...
        <div className="px-5 py-4 bg-slate-950 border-b border-slate-800 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="px-2 py-0.5 rounded-full bg-indigo-950 text-indigo-300 border border-indigo-700/50 text-[10px] uppercase font-bold tracking-wider">
              {product.category}
            </span>
            {product.badge && (
        ...
        <div className="p-6 overflow-y-auto space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="relative rounded-2xl overflow-hidden border border-slate-800 bg-slate-950 aspect-square">
              ...
              />
              <div className="absolute bottom-3 left-3 bg-slate-950/80 backdrop-blur-md px-2.5 py-1 rounded-lg border border-slate-700/50 flex items-center gap-1.5 text-xs text-amber-300">
            ...
            <div className="space-y-4">
              <div>
                ...
                <p className="text-xs text-indigo-300 font-medium mt-0.5">{product.tagline}</p>
              </div>
          ...
          <div className="space-y-2">
            <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Technical Specifications</span>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
              {Object.entries(product.specs).map(([key, val]) => (
                <div key={key} className="p-2.5 rounded-xl bg-slate-950/70 border border-slate-800/80 flex justify-between">
          ...
          {product.crossSellReason && (
            <div className="p-4 rounded-xl bg-indigo-950/30 border border-indigo-800/40 space-y-1.5">
              ...
              <p className="text-xs text-slate-300">{product.crossSellReason}</p>
            </div>
          )}
        </div>
        ...
        <div className="px-6 py-4 bg-slate-950 border-t border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            <button
              ...
              className="flex-1 sm:flex-none px-4 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-semibold flex items-center justify-center gap-1.5 transition-colors"
            >
              <MessageSquare className="w-3.5 h-3.5 text-indigo-400" />
              <span>Ask AlphaCart</span>
            </button>
            ...
            <button
              ...
              className="flex-1 sm:flex-none px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold flex items-center justify-center gap-1.5 transition-all shadow-lg shadow-indigo-900/40"
            >

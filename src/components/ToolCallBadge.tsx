...
export const ToolCallBadge: React.FC<ToolCallBadgeProps> = ({ toolCall }) => {
  ...
  return (
    <div id={`tool-call-${toolCall.id}`} className="my-2 rounded-xl border border-slate-800 bg-slate-900/80 overflow-hidden text-xs shadow-md">
      <button
        id={`btn-toggle-tool-${toolCall.id}`}
        onClick={() => setIsOpen(!isOpen)}
        className="w-full px-3.5 py-2.5 flex items-center justify-between hover:bg-slate-800/60 transition-colors text-left"
      >
        <div className="flex items-center gap-2 overflow-hidden">
          ...
          {toolCall.executionGatePassed && (
            <span className="hidden sm:inline-flex items-center gap-1 px-1.5 py-0.5 rounded bg-emerald-900/40 text-emerald-300 border border-emerald-700/50 text-[10px]">
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          {toolCall.status === 'success' && <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" />}
      ...
      {isOpen && (
        <div className="p-3 border-t border-slate-800 bg-slate-950/70 space-y-2.5">
          {toolCall.securityNote && (
            <div className="p-2 rounded bg-indigo-950/40 border border-indigo-800/40 text-indigo-200 text-[11px] flex items-center gap-1.5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-[11px] font-mono">
            <div className="space-y-1">
              <span className="text-slate-400 uppercase tracking-wider font-sans font-bold text-[10px]">Input Payload</span>
              <pre className="p-2 rounded bg-slate-900 border border-slate-800 text-slate-300 overflow-x-auto max-h-40">
                {JSON.stringify(toolCall.input, null, 2)}
              </pre>
            </div>
            <div className="space-y-1">
              <span className="text-slate-400 uppercase tracking-wider font-sans font-bold text-[10px]">Tool Output</span>
              <pre className="p-2 rounded bg-slate-900 border border-slate-800 text-emerald-300 overflow-x-auto max-h-40">

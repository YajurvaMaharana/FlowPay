...
// 1. Fix the `handleSendMessage(..., true)` calls
code = code.replace(
...
// 2. We'll extract the old processUserMessage block and conditionally route to vision
...
const regex = /\/\/ Handle User Message Submission[\s\S]*?finally \{\s*setIsLoading\(false\);\s*\}\s*\};/m;
code = code.replace(regex, newHandleSendMessage.trim());
